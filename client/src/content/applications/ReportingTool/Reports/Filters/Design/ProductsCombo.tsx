...
const DropdownProductFilter: React.FC<DropdownProductFilterProps> = ({
  filter,
  onChange,
}) => {
  const handleChange = (
  ...
  return (
    ...
    style={{ margin: "8px", minWidth: 300,  maxWidth:600 }}
      ...
      options={filter.options as ProductModel[]}
      getOptionLabel={(option) => `${option.code} - ${option.name}`}
      value={(filter.options as ProductModel[])?.filter(
...
export default DropdownProductFilter;
