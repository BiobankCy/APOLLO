...
const DropdownFilter: React.FC<DropdownProductFilterProps> = ({
  ...
  const handleChange = (
    ...
    selectedProducts: TenderModel[] | null
  ) => {
    if (selectedProducts) {
  ...
  return (
    ...
    style={{ margin: "8px", minWidth: 300,  maxWidth:600 }}
      ...
      options={filter.options as TenderModel[]}
      getOptionLabel={(option) => `${option.tendercode}`}
      value={(filter.options as TenderModel[])?.filter(
