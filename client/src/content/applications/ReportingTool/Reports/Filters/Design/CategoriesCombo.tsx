...
import { CategoryModel } from "../../../../../../models/mymodels";
...
const DropdownFilter: React.FC<DropdownProductFilterProps> = ({
  ...
  const handleChange = (
    ...
    selectedProducts: CategoryModel[] | null
  ) => {
    if (selectedProducts) {
  ...
  return (
    ...
    style={{ margin: "8px", minWidth: 300,  maxWidth:600 }}
      ...
      options={filter.options as CategoryModel[]}
      getOptionLabel={(option) => `${option.name}`}
      value={(filter.options as CategoryModel[])?.filter(
